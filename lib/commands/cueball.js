...
export async function createCueball(name) {
  ...
  const cueballsDir = join(projectRoot, 'public/cueballs');
  ...
  try {
    ...
    const cssDir = join(projectRoot, 'public/css');
    ...
    console.log(chalk.green('✓') + chalk.gray(` Created public/cueballs/${kebabName}.html`));
    
    writeFileSync(join(cssDir, `${kebabName}.css`), css);
    console.log(chalk.green('✓') + chalk.gray(` Created public/css/${kebabName}.css`));
    
    writeFileSync(join(jsDir, `${kebabName}.js`), js);
    console.log(chalk.green('✓') + chalk.gray(` Created public/js/${kebabName}.js`));
