...
export class WebSocketManager {
  ...
  private webpackServer: WebSocketServer;
  ...
  public handleUpgrade(request: IncomingMessage, socket: Duplex, head: Buffer): void {
    ...
    if (pathname === '/cuestation') {
      ...
      // Forward on to localhost:1212 (webpack dev server)
      const proxySocket = net.connect(1212, 'localhost', () => {
        proxySocket.write(head);
        socket.pipe(proxySocket).pipe(socket);
      });
      
      proxySocket.on('error', (err: Error) => {
        this.log(`Proxy socket error: ${err.message}`, 'error');
        socket.destroy();
      });
      
      socket.on('error', (err: Error) => {
        this.log(`Client socket error: ${err.message}`, 'error');
        proxySocket.destroy();
      });
    } else {
      socket.destroy();
