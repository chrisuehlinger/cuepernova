...
declare const Maptastic: (config: {
  layers?: string[];
...
$(async () => {
  ...
  const $mappingFrame = $(
    `<iframe src="${embedUrl}" id="mapping-frame" frameborder="0" width="${window.innerWidth}" height="${window.innerHeight}"></iframe>`
  );
  ...
  window.addEventListener(
    ...
    (e: MessageEvent<MappingMessage>) => {
      ...
      switch (message.type) {
        ...
        default:
          console.log(
            `No message handler for message of type: ${message.type}`
          );
      }
    },
    false
